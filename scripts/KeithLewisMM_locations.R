...
# Title: "Labrador Highlands Research Group Point Counts"
# Source dataset for this script are in different .xls files
# Author: "Ana Raymundo"
# Date: "February 2nd, 2023"
# ---Notes:
# fix problems with date format
...
source_data <- "P9 I1 C1 Keith Lewis Mealy Mountains.xls"
project <- 'KeithLewisMM'
...
pc_location <- raw_location %>%  dplyr::select('site_id', 'station_id', 'date_________(yyyy-mm-dd)',
                                               'northing','easting') %>% 
  rename('date' = 'date_________(yyyy-mm-dd)') %>%  
  unite(location , site_id:station_id, remove = FALSE)

# fix the problem of the date 
pc_location <- pc_location %>% mutate(date = as.Date(as.numeric(date), origin = '1899-12-30')) %>% 
...
survey_summ <- left_join(pc_location, pc_visit, by = c('location','site_id', 
                                                       'station_id', 'date', 
                                                       'year','month', 'day'))
...
#### MAKE MAP ####
############################
# database CRS (NAD83, UTM ZONES 21, EPSG: 26921)
crs_db <- st_crs(26921)
