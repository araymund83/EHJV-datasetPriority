#PCODE: RaptorMP1998
# Title: "Raptor Monitoring Program 1998
...
source_data <- "P16 I1 C1 1998 Raptor monitoring program.xls"
project <- 'RaptorMP1998'
...
pc_visit <- raw_visit %>%  dplyr::select('site_id','station_id', 'date_________(yyyy-mm-dd)',
                                         'species', 'count', 'latitude', 'longitude') %>% 
...
survey_summ <- right_join(pc_location, pc_visit, by = c('location', 'date', 'year',
                                                       'month', 'day'))
