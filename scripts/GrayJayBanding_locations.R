#PCODE: GrayJayBanding
# Title: "Gray Jay (Perisoreus Canadensis) 2003 fall banding program"
...
source_data <- "P15 I1 C1 Gray Jay.xls"
project <- 'GrayJayBanding'
...
pc_visit <- raw_visit %>%  dplyr::select('site_id','station_id', 'date_________(yyyy-mm-dd)',
                                         'species', 'count', 'easting', 'northing') %>% 
...
survey_summ <- left_join(pc_location, pc_visit, by = c('location', 'date', 'year',
                                                       'month', 'day','easting',
                                                       'northing'))
